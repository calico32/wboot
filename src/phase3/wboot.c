...
static const CHAR8 cmdline[] = "rw root=/dev/vda2 loglevel=3";
...
wboot_locate_kernel(
    ...
    printf(L"    [locate_kernel] locating kernel...\r\n");
    ...
    if (EFI_ERROR(status)) {
        perror(L"    [locate_kernel] error locating handles", status);
        return status;
    }

    printf(
        L"    [locate_kernel] enumerating SIMPLE_FILE_SYSTEM_PROTOCOL devices\r\n",
    ...
    for (UINTN i = 0; i < handles_size / sizeof(EFI_HANDLE); i++) {
        ...
        if (!EFI_ERROR(status)) {
            ...
            if (device_path_str == NULL) {
                printf(L"    [locate_kernel]   device path: unknown error\r\n");
            }
            printf(L"    [locate_kernel]   device path: %s\r\n", device_path_str);
            BS->FreePool(device_path_str);
        } else {
            perror(L"    [locate_kernel]   error getting device path", status);
        }
        ...
        if (EFI_ERROR(status)) {
            perror(L"    [locate_kernel]   error getting file system protocol", status);
            continue;
        }
        ...
        if (EFI_ERROR(status)) {
            perror(L"    [locate_kernel]   error opening volume", status);
            continue;
        }
        ...
        if (EFI_ERROR(status)) {
            perror(L"    [locate_kernel]   error opening vmlinuz-linux", status);
            continue;
        }
        printf(L"    [locate_kernel]   found vmlinuz-linux!\r\n");
        *kernel_file = kernel;
        ...
        if (EFI_ERROR(status)) {
            perror(L"    [locate_kernel]   error opening initramfs-linux.img", status);
            continue;
        }
        printf(L"    [locate_kernel]   found initramfs-linux.img!\r\n");
    ...
    printf(
        L"    [locate_kernel] could not find vmlinuz-linux and initramfs-linux.img "
        L"together on any device\r\n"
    );
...
EFI_STATUS wboot_decompress_kernel(
    ...
    if (dctx == NULL) {
        ...
        if (dctx == NULL) {
            printf(
                L"    [decompress_kernel] error creating ZSTD decompression "
                L"context\r\n"
            );
    ...
    if (compressed_kernel == NULL) {
        printf(
            L"    [decompress_kernel] error allocating memory for compressed "
            L"kernel\r\n"
        );
        return EFI_OUT_OF_RESOURCES;
    }
    ...
    printf(
        L"    [decompress_kernel] kernel setup size: %u bytes (%u setup sectors)\r\n",
        sects * 512, sects
    );
    UINTN payload_start = ((1 + sects) * 512) + header->payload_offset;

    printf(
        L"    [decompress_kernel] seeking to kernel payload at offset 0x%x and "
        L"reading %u bytes of compressed "
        L"kernel\r\n",
    ...
    if (EFI_ERROR(status)) {
        printf(L"    [decompress_kernel] error setting kernel file position\r\n");
        free(compressed_kernel);
        return status;
    }
    status = kernel_file->Read(kernel_file, &bytes_read, compressed_kernel);
    if (EFI_ERROR(status)) {
        printf(L"    [decompress_kernel] error reading compressed kernel\r\n");
        free(compressed_kernel);
        return status;
    }
    ...
    if (compressed_kernel_size < 4 || *((UINT32 *)compressed_kernel) != 0xFD2FB528) {
        printf(
            L"    [decompress_kernel] error: compressed kernel does not have zstd "
            L"magic (different compression format or not compressed at all?) expected "
    ...
    if (ZSTD_isError(exact_compressed_size)) {
        printf(
            L"    [decompress_kernel] error finding zstd compressed frame size: "
            L"%-s\r\n",
    ...
    printf(
        L"    [decompress_kernel] exact zstd compressed size: %u bytes\r\n",
    ...
    printf(
        L"    [decompress_kernel] decompressed kernel size: %u bytes, which requires "
        L"%u pages\r\n",
    ...
    if (header->pref_address != 0) {
        ...
        printf(
            L"    [decompress_kernel] Asking for %u bytes for decompressed kernel at "
            L"preferred address "
            L"0x%x\r\n",
        ...
        if (EFI_ERROR(status)) {
            printf(
                L"    [decompress_kernel] Error allocating pages for decompressed "
                L"kernel at preferred address "
                L"0x%x: %s\r\n",
    ...
    printf(
        L"    [decompress_kernel] Decompressing kernel to address %p...\r\n",
...
EFI_STATUS wboot_load_initramfs(
    ...
    printf(
        L"    [load_initramfs] allocating %u bytes for initramfs file info\r\n", size
    );
    ...
    if (buffer == NULL) {
        printf(
            L"    [load_initramfs] Error allocating memory for initramfs: %s\r\n",
            strerror(errno)
        );
        return errno;
    }
...
boot_params_t *wboot_prepare_bootparams(
    ...
    params->ext_cmd_line_ptr = (UINT32)((UINTN)cmdline >> 32);
    params->hdr.cmdline_size = sizeof(cmdline) - 1; // exclude null terminator
...
static void populate_bootparams_efi(
    ...
    printf(L"    [populate_bootparams_efi] populating EFI info in boot params\r\n");
...
EFI_STATUS wboot_allocate_e820(
    ...
    if (EFI_ERROR(status)) {
        printf(
            L"    [allocate_e820] error getting memory map: %s\r\n", strerror(status)
        );
        free(memmap);
        return status;
    }
...
EFI_MEMORY_DESCRIPTOR *wboot_exit_bs(boot_params_t *params) {
    ...
    printf(L"    [exit_bs] getting memory map size\r\n");
    EFI_STATUS status = BS->GetMemoryMap(
        &map_size, NULL, &map_key, &desc_size, &desc_ver
    );
    if (status != EFI_BUFFER_TOO_SMALL) {
        printf(
            L"    [exit_bs] error getting memory map size: %s\r\n", strerror(status)
        );
        return NULL;
    }
    ...
    if (memmap == NULL) {
        printf(
            L"    [exit_bs] error allocating memory for memory map: %s\r\n",
            strerror(errno)
        );
        return NULL;
    }

    printf(L"    [exit_bs] getting memory map\r\n");
    status = BS->GetMemoryMap(&map_size, memmap, &map_key, &desc_size, &desc_ver);
    if (EFI_ERROR(status)) {
        printf(L"    [exit_bs] error getting memory map: %s\r\n", strerror(status));
        free(memmap);
        return NULL;
    }

    printf(L"    [exit_bs] exiting boot services\r\n");
    status = BS->ExitBootServices(ImageHandle, map_key);
    if (EFI_ERROR(status)) {
        printf(L"    [exit_bs] error exiting boot services: %s\r\n", strerror(status));
        ...
        if (EFI_ERROR(status)) {
            printf(
                L"    [exit_bs] error getting memory map on second attempt: %s\r\n",
                strerror(status)
            );
            free(memmap);
            return NULL;
        ...
        if (EFI_ERROR(status)) {
            printf(
                L"    [exit_bs] error exiting boot services on second attempt: %s\r\n",
                strerror(status)
            );
            free(memmap);
            return NULL;
    ...
    wstdlib_bs_exited();
    printf(L"    [exit_bs] successfully exited boot services\r\n");
...
VOID *wboot_get_kernel_entry(const VOID *decompressed_kernel) {
    printf(
        L"    [get_kernel_entry] parsing ELF headers to find kernel entry point\r\n"
    );
